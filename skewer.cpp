...
//    mask(Rect(0,dims.height/4,dims.width/4,(3*dims.height)/4)) = 1; 
	...
	SIFT lift(common, detector, descriptor);

	cv::Mat img1_descriptors, img2_descriptors;

    lift.setScaleType(atoi(argv[4]));
    lift(img1, mask, img1_points, img1_descriptors);
    lift(img2, mask, img2_points, img2_descriptors);
    long count = 0;
    double tol = 5E-1;
